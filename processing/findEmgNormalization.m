...
function findEmgNormalization(varargin)
    ...
    emg_variables = subject_settings.emg_labels;
    data_custodian = WalkingDataCustodian();
    number_of_stretch_variables = length(emg_variables);
    ...
    for i_condition = 1 : length(condition_list)
        ...
        for i_trial = trials_to_process
            ...
            data_custodian.prepareBasicVariables(condition, i_trial, [{'emg_trajectories'}, subject_settings.emg_labels]);
            
            load(['analysis' filesep makeFileName(date, subject_id, condition, i_trial, 'relevantDataStretches')]);
            data_trial = data_custodian.calculateStretchVariables(stretch_start_times, stretch_end_times, condition_stance_foot_list_trial, emg_variables);
    ...
    number_of_conditions_emg_normalization = size(study_settings.conditions_emg_normalization, 1);
    ...
    for i_condition = 1 : number_of_conditions_emg_normalization
        stance_foot_indicator = strcmp(condition_stance_foot_list_subject, study_settings.conditions_emg_normalization(i_condition, 1));
        perturbation_indicator = strcmp(condition_perturbation_list_subject, study_settings.conditions_emg_normalization(i_condition, 2));
        delay_indicator = strcmp(condition_delay_list_subject, study_settings.conditions_emg_normalization(i_condition, 3));
        index_indicator = strcmp(condition_index_list_subject, study_settings.conditions_emg_normalization(i_condition, 4));
        experimental_indicator = strcmp(condition_experimental_list_subject, study_settings.conditions_emg_normalization(i_condition, 5));
        stimulus_indicator = strcmp(condition_stimulus_list_subject, study_settings.conditions_emg_normalization(i_condition, 6));
        day_indicator = strcmp(condition_day_list_subject, study_settings.conditions_emg_normalization(i_condition, 7));
    ...
    conditions_emg_normalization_with_number = study_settings.conditions_emg_normalization;
    for i_condition = 1 : number_of_conditions_emg_normalization
        conditions_emg_normalization_with_number{i_condition, size(study_settings.conditions_emg_normalization, 2)+1} = num2str(trials_per_condition_emg_normalization(i_condition));
    end
    conditions_emg_normalization_with_labels = [study_settings.condition_labels 'number of stretches'; conditions_emg_normalization_with_number];
    disp('EMG normalization conditions:')
    disp(conditions_emg_normalization_with_labels);

    % average across stretches
    emg_normalization_values = zeros(length(emg_variables), 1);
    for i_variable = 1 : number_of_stretch_variables
