...
function processStretchVariables(varargin)
    day_folder_list = determineDataStructure;
    
    for i_folder = 1 : length(day_folder_list)
        % load data
        data_path = day_folder_list{i_folder};
        load([data_path filesep 'subjectInfo.mat'], 'date', 'subject_id');
        % need a way to process individual day folders and then cache while
        % move on to process the next day. Cocatenate the days into a
        % single results.mat. Is this ok in the event you want to exclude a
        % day? Should be ok so long as stored in conidition_day_list
