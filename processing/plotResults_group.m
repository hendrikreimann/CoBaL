...
function plotResults(varargin)
    ...
    addParameter(parser, 'settings', 'plotSettings_group.txt')
    ...
    % load settings
    study_settings_file = '';
    if exist('studySettings_group.txt', 'file')
        study_settings_file = 'studySettings_group.txt';
    end    
    if exist(['..' filesep 'studySettings_group.txt'], 'file')
        study_settings_file = ['..' filesep 'studySettings_group.txt'];
    end    
    if exist(['..' filesep '..' filesep 'studySettings_group.txt'], 'file')
        study_settings_file = ['..' filesep '..' filesep 'studySettings_group.txt'];
    end
    ...
    for i_folder = 1 : length(data_folder_list)
        ...
        subject_list_session = repmat(data_folder_list{i_folder}(end-2:end),length(condition_stance_foot_list_session),1);
        subject_list_all = [subject_list_all; subject_list_session];
...
%                         abscissae_cell{i_comparison, i_variable}(i_condition, :) = abscissae;
                ...
                if isContinuousVariable(i_variable, variable_data_all)
                    ...
                    for i_condition = 1 : length(conditions_this_comparison)
                        ...
                        if plot_settings.get('plot_total_response')
                        ...
                        if plot_settings.get('plot_groups') & strcmp(condition_identifier{4}, 'ONE')
                            stance_foot_checks = {'left', 'right'};
                             if strcmp(condition_identifier{8},'LEFT')
                                foot_assignment = plot_settings.get('foot_assignment_sides');
                                foot_assignment_column = 2;
                             elseif strcmp(condition_identifier{8},'RIGHT')
                                foot_assignment = plot_settings.get('foot_assignment_sides');
                                foot_assignment_column = 3;
                            elseif strcmp(condition_identifier{8},'EARLY')
                                foot_assignment = plot_settings.get('foot_assignment_onset');
                                foot_assignment_column = 2;
                            elseif strcmp(condition_identifier{8},'LATE')
                                foot_assignment = plot_settings.get('foot_assignment_onset');
                                foot_assignment_column = 3;
                            elseif strcmp(condition_identifier{8},'NO')
                                foot_assignment = plot_settings.get('foot_assignment_onset');
                            ...
                            subject_indices = find(strcmp(foot_assignment(:,foot_assignment_column),stance_foot_checks(this_foot)) | strcmp(foot_assignment(:,foot_assignment_column),'both'));
                            this_group_subjects(:,i_condition) = {foot_assignment(subject_indices,1)};
                        end
                       
                        if strcmp(condition_identifier{8}, 'LEFT') | strcmp(condition_identifier{8}, 'EARLY')
                            subject_indicator = ismember(subject_list_all,this_group_subjects{:,1});
                        elseif strcmp(condition_identifier{8}, 'RIGHT') | strcmp(condition_identifier{8}, 'LATE')
                            subject_indicator = ismember(subject_list_all,this_group_subjects{:,2});
                        elseif strcmp(condition_identifier{8}, 'NO') 
                            subject_indicator = ismember(subject_list_all,this_group_subjects{:,3});
                        else 
                            subject_indicator = ones(length(subject_list_all),1);
                        end
                        ...
                        this_condition_indicator = stance_foot_indicator & perturbation_indicator & delay_indicator & index_indicator & experimental_indicator & stimulus_indicator & day_indicator & subject_indicator;
    ...
    for i_variable = 1 : number_of_variables_to_plot
        ...
        for i_comparison = 1 : length(comparison_indices)
            ...
            if plot_settings.get('plot_control') && ~isempty(conditions_control) && ~plot_settings.get('plot_response')
                ...
                perturbation_indicator = strcmp(condition_perturbation_list_all, applicable_control_condition_labels{3});
...
%                                     data_to_plot_this_condition, ...
            ...
            for i_condition = 1 : length(conditions_this_comparison)
                ...
                if plot_settings.get('plot_total_response')
                    perturbation_indicator_left = strcmp(condition_perturbation_list_all,'ILLUSION_LEFT');
                    perturbation_indicator_right = strcmp(condition_perturbation_list_all,'ILLUSION_RIGHT'); 
                else
                    perturbation_indicator = strcmp(condition_perturbation_list_all, condition_identifier{2});
                end
                ...
                day_indicator = strcmp(condition_day_list_all, condition_identifier{7});
                if plot_settings.get('plot_total_response')
                    this_condition_indicator_left = stance_foot_indicator & perturbation_indicator_left & delay_indicator & index_indicator & experimental_indicator & stimulus_indicator & day_indicator & subject_indicator;
                    this_condition_indicator_right = stance_foot_indicator & perturbation_indicator_right & delay_indicator & index_indicator & experimental_indicator & stimulus_indicator & day_indicator & subject_indicator;
                    this_condition_indicator = stance_foot_indicator & perturbation_indicator_right & perturbation_indicator_left & delay_indicator & index_indicator & experimental_indicator & stimulus_indicator & day_indicator & subject_indicator;
                    data_to_plot_left_inverted = -data_to_plot(:, this_condition_indicator_left);
                    data_to_plot_right = data_to_plot(:, this_condition_indicator_right);
                    data_to_plot_this_condition = [data_to_plot_left_inverted data_to_plot_right];
                else    
                    this_condition_indicator = stance_foot_indicator & perturbation_indicator & delay_indicator & index_indicator & experimental_indicator & stimulus_indicator & day_indicator & subject_indicator;
                    data_to_plot_this_condition = data_to_plot(:, this_condition_indicator);
                end
