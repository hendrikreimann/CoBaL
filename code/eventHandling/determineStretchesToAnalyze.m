...
function determineStretchesToAnalyze(varargin)
    ...
    for i_trial = 1 : height(trials_to_process_table)
        ...
        % load data
        trial_data.loaded_events_data = load(['analysis' filesep ...
            makeFileName(collection_date, subject_id, trial_data.trial_type, trial_data.trial_number, 'events')]);
        ...
        trial_data.condition_experimental = getExperimentalCondition(study_settings, subject_settings, trial_data); 
        % the "experimental condition" is a specially highlighted condition that will be used to make some decisions 
        % lateron
        ...
        [conditions_trial, event_variables_to_save] ...
            = removeLevelsSpecifiedInSettings(study_settings, conditions_trial, event_variables_to_save);
...
function trial_data = loadKinematicData(study_settings, subject_settings, trial_data)
    ...
    [trial_data.marker_trajectories, trial_data.time_marker, ~, trial_data.marker_labels] ...
        = loadData ...
          ( ...
            collection_date, ...
            subject_id, ...
    ...
    if study_settings.get('prune_gaps_angles', 1)
        trial_data.joint_angle_trajectories ...
            = loadData ...
              ( ...
                collection_date, ...
                subject_id, ...
...
function trial_data = loadForceplateData(study_settings, subject_settings, trial_data)
    ...
    % 2021-05-02, HR:
    % these data don't exist anymore in this form, so removing here to see what exactly we need elsewhere
    % this happened on CP phase 2 data. Removing this didn't throw any errors, so I'm leaving it out for now
    % if the CoP data is required for some other paradigm, it should throw an error there
    % When that happens, go back here and load the CoP data in the new format
    
    % forceplate data
%     trial_data.left_forceplate_cop_world_trajectory ...
%         = loadData ...
%           ( ...
%             collection_date, ...
%             subject_id, ...
...
%             'optional' ...
%           );
%     trial_data.right_forceplate_cop_world_trajectory ...
%         = loadData ...
%           ( ...
%             collection_date, ...
%             subject_id, ...
...
%             'optional' ...
%           );
%     [trial_data.cop_world_trajectory, trial_data.time_forceplate] ...
%         = loadData ...
%           ( ...
%             collection_date, ...
%             subject_id, ...
...
%           );
    if strcmp(experimental_paradigm, 'GvsOverground')
        first_forceplate_wrench_trajectory ...
            = loadData ...
              ( ...
                collection_date, ...
                subject_id, ...
...
function trial_data = loadStimulusData(study_settings, subject_settings, trial_data)
    ...
    if strcmp(experimental_paradigm, 'GVS_old')
        [trial_data.stimulus_state_trajectory, trial_data.time_stimulus] ...
            = loadData ...
              ( ...
                collection_date, ...
                subject_id, ...
                trial_data.trial_type, ...
                trial_data.trial_number, ...
                'stimulus_state_trajectory' ...
              );
        labview_data_filename = ...
          [ ...
            'processed' filesep ...
            makeFileName(collection_date, subject_id, trial_data.trial_type, trial_data.trial_number, 'labviewData') ...
          ];
        loaded_labview_data = load(labview_data_filename);
    ...
    if strcmp(experimental_paradigm, 'Vision_old')
        % this if for TU data
        trial_data.visual_scene_ml_translation_trajectory ...
            = loadData ...
              ( ...
                collection_date, ...
                subject_id, ...
                trial_data.trial_type, ...
                trial_data.trial_number, ...
                'visual_scene_ml_translation__trajectory'  ... %take note of the double "_"
              );
        [stimulus_state_trajectory, time_stimulus] ...
            = loadData ...
              ( ...
                collection_date, ...
                subject_id, ...
...
%         current_rotation_trajectory = loadData(collection_date, subject_id, trial_data.trial_type, trial_data.trial_number, 'visual_rotation_angle_trajectory');
        trial_data.current_acceleration_trajectory = ...
            loadData ...
              ( ...
                collection_date, ...
                subject_id, ...
                trial_data.trial_type, ...
                trial_data.trial_number, ...
                'visual_rotation_acceleration_trajectory' ...
              );
        trial_data.current_rotation_trajectory ...
            = loadData ...
              ( ...
                collection_date, ...
                subject_id, ...
                trial_data.trial_type, ...
                trial_data.trial_number, ...
                'visual_rotation_angle_trajectory' ...
              );
        [stimulus_state_trajectory, time_stimulus] ...
            = loadData ...
              ( ...
    ...
    if strcmp(experimental_paradigm, 'GvsOverground')
        [analog_trajectories, time_analog, ~, analog_labels, ~] ...
            = loadData ...
              ( ...
                collection_date, ...
                subject_id, ...
        ...
        scene_translation_trajectory ...
            = loadData ...
              ( ...
                ...
                trial_data.trial_type, ...
                trial_data.trial_number, ...
                'SceneTranslation_trajectory' ...
              );
        virtual_object_info = load('virtualobjectInfo');
        trial_data.scene_translation_trajectory = scene_translation_trajectory;
        trial_data.virtual_object_ap_location = virtual_object_info.virtual_object_ap_location;
        trial_data.virtual_object_ml_location = virtual_object_info.virtual_object_ml_location;
        trial_data.stimulus_state_trajectory = stimulus_state_trajectory;
        trial_data.time_stimulus = time_stimulus;
    end
...
function trial_data = extractEvents(study_settings, trial_data)
    ...
    walking_paradigms = ...
      { ...
        'GVS_old', 'Vision_old',  ...
        'Vision', 'CadenceVision', 'SR_VisualStim', 'CognitiveLoadVision', ...
        'GVS', 'CadenceGVS', 'FatigueGVS', 'CognitiveLoadGvs', ...
        'OBSTACLE', 'ARMSENSE', 'GaitInitiationObstacle', 'Vision Stochastic', 'GvsOverground'...
        'OculusLaneRestriction', 'nGVS_Vision' ...
      };
    if any(strcmp(experimental_paradigm, walking_paradigms))
...
function [conditions_trial, event_variables_to_save] ...
    ...
    if study_settings.get('prune_gaps_com', 1)
        for i_stretch = 1 : number_of_stretches
            [~, start_index_mocap] ...
                = min(abs(trial_data.time_marker - event_variables_to_save.stretch_times(i_stretch, 1)));
            [~, end_index_mocap] ...
                = min(abs(trial_data.time_marker - event_variables_to_save.stretch_times(i_stretch, end)));
    ...
    if strcmp(experimental_paradigm, 'OculusLaneRestriction')
        ...
        if study_settings.get('prune_step_placements')
            ...
            for i_stretch = 1 : number_of_stretches
                [~, trigger_start_index_mocap] ...
                    = min(abs(trial_data.time_marker - event_variables_to_save.stretch_times(i_stretch, 1)));
                [~, trigger_end_index_mocap] ...
                    = min(abs(trial_data.time_marker - event_variables_to_save.stretch_times(i_stretch, 2)));
                [~, remainder_start_index_mocap] ...
                    = min(abs(trial_data.time_marker - event_variables_to_save.stretch_times(i_stretch, 1)));
                [~, remainder_end_index_mocap] ...
                    = min(abs(trial_data.time_marker - event_variables_to_save.stretch_times(i_stretch, 2)));
                ...
                if strcmp(conditions_trial.zone_side_list{i_stretch}, 'STIM_ZONE_LEFT')
                    ...
                    if any(LHEE_marker_data(trigger_start_index_mocap : trigger_end_index_mocap,1) < threshold) ...
                      || any(RHEE_marker_data(trigger_start_index_mocap : trigger_end_index_mocap,1) < threshold) ...
                      || any(RTOE_marker_data(trigger_start_index_mocap : trigger_end_index_mocap,1) < threshold) ...
                      || any(LTOE_marker_data(trigger_start_index_mocap : trigger_end_index_mocap,1) < threshold)

                        step_zone_delinquent_list{i_stretch} = 'TRIGGER';
                        disp('Stretch flagged due stepping in No Step Zone')
                    end
                    if any(LHEE_marker_data(remainder_start_index_mocap : remainder_end_index_mocap,1) < threshold) ...
                      || any(RHEE_marker_data(remainder_start_index_mocap : remainder_end_index_mocap,1) < threshold) ...
                      || any(RTOE_marker_data(remainder_start_index_mocap : remainder_end_index_mocap,1) < threshold) ...
                      || any(LTOE_marker_data(remainder_start_index_mocap : remainder_end_index_mocap,1) < threshold)
    ...
    for i_marker = 1 : 3 : length(trial_data.marker_labels)
        ...
        if this_marker_weight == 1
            this_marker_data ...
                = extractMarkerData(trial_data.marker_trajectories, trial_data.marker_labels, this_marker_label);
            for i_stretch = 1 : number_of_stretches
                [~, start_index] ...
                    = min(abs(trial_data.time_marker - event_variables_to_save.stretch_times(i_stretch, 1)));
                [~, end_index] ...
                ...
                if any(any(isnan(this_marker_data(start_index : end_index, :))))
                    removal_flags(i_stretch) = 1;
                    stretch_start_time = trial_data.time_marker(start_index);
                    stretch_end_time = trial_data.time_marker(end_index);
                    disp ...
                      ( ...
                        [ ...
                          'Removing a stretch due to gap in marker "' ...
                          this_marker_label ...
                          '" (' num2str(stretch_start_time) '-' num2str(stretch_end_time) 's)'
                        ] ...
                      );
                end 
            end
    ...
    for i_stretch = 1 : number_of_stretches
        if isfield(trial_data.loaded_events_data, 'ignore_times')
            for i_ignore = 1 : length(trial_data.loaded_events_data.ignore_times)
                if event_variables_to_save.stretch_times(i_stretch, 1) ...
                    <= trial_data.loaded_events_data.ignore_times(i_ignore) ...
                  && trial_data.loaded_events_data.ignore_times(i_ignore) ...
                    <= event_variables_to_save.stretch_times(i_stretch, end)
    ...
    stretches_file_name ...
        = makeFileName...
          ( ...
            ...
            trial_data.trial_type, ...
            trial_data.trial_number, ...
            'relevantDataStretches' ...
          );
    saveDataToFile(['analysis' filesep stretches_file_name], event_variables_to_save);


    disp ...
      ( ...
        [ ...
          'Finding Relevant Data Stretches: condition ' char(trial_data.trial_type) ...
          ', Trial ' num2str(trial_data.trial_number) ...
          ' completed, found ' num2str(size(event_variables_to_save.stretch_times, 1)) ...
          ' relevant stretches, saved as ' stretches_file_name ...
        ] ...
      );
end
