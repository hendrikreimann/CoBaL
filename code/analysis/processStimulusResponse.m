...
function processStimulusResponse(varargin)
    ...
    fy_data = loaded_data.stretch_data_session{strcmp(loaded_data.stretch_names_session, 'fy')};
    fz_data = loaded_data.stretch_data_session{strcmp(loaded_data.stretch_names_session, 'fz')};
    cop_data = loaded_data.stretch_data_session{strcmp(loaded_data.stretch_names_session, 'cop_x')};
    ...
    for i_condition = 1 : number_of_conditions_control
        ...
        for i_stretch = 1 : length(this_condition_indices)
            ...
            this_stretch_fy_data = fy_data(:, this_condition_indices(i_stretch));
            this_stretch_fz_data = fz_data(:, this_condition_indices(i_stretch));
            this_stretch_cop_data = cop_data(:, this_condition_indices(i_stretch));
            ...
            for i_band = 1 : number_of_bands_per_stretch
                if ~isnan(this_stretch_step_placement_x_data(i_band))
                    ...
                    fy_control_data = [fy_control_data; this_stretch_fy_data(end)]; %#ok<AGROW>
                    fz_control_data = [fz_control_data; this_stretch_fz_data(end)]; %#ok<AGROW>
                    cop_control_data = [cop_control_data; this_stretch_cop_data(end)]; %#ok<AGROW>
    ...
    for i_stance = 1 : length(stance_feet)
        ...
        this_stance_fy_data = fy_control_data(this_stance_foot_indicator);
        this_stance_fz_data = fz_control_data(this_stance_foot_indicator);
        this_stance_cop_data = cop_control_data(this_stance_foot_indicator);
        ...
        fy_means(i_stance) = mean(this_stance_fy_data);
        fz_means(i_stance) = mean(this_stance_fz_data);
        cop_means(i_stance) = mean(this_stance_cop_data);
        
        this_stance_fx_data_mean_free = this_stance_fx_data - fx_means(i_stance);
        this_stance_fy_data_mean_free = this_stance_fy_data - fy_means(i_stance);
        this_stance_fz_data_mean_free = this_stance_fz_data - fz_means(i_stance);
        this_stance_cop_data_mean_free = this_stance_cop_data - cop_means(i_stance);
        ...
        % fit fx regression model
        [fit_object, fit_stats] = fit([this_stance_com_from_stance_ankle_data_mean_free, this_stance_com_x_vel_midstance_data_mean_free], this_stance_fx_data_mean_free, 'poly11');
        fx_linear_models{i_stance} = [fit_object.p10, fit_object.p01];

        if visualize
            figure; 
            plot(fit_object, [this_stance_com_from_stance_ankle_data_mean_free, this_stance_com_x_vel_midstance_data_mean_free], this_stance_fx_data_mean_free)
            xlabel('\Delta CoM from stance ankle'); ylabel('\Delta CoM vel'); zlabel('\Delta foot placement')
            title(['slopes = [' num2str(linear_model_slopes(1)) ', ' num2str(linear_model_slopes(2)) '], r^2 = ' num2str(fit_stats.rsquare)]);
        end    
        
        % fit fy regression model
        [fit_object, fit_stats] = fit([this_stance_com_from_stance_ankle_data_mean_free, this_stance_com_x_vel_midstance_data_mean_free], this_stance_fy_data_mean_free, 'poly11');
        fy_linear_models{i_stance} = [fit_object.p10, fit_object.p01];

        if visualize
            figure; 
            plot(fit_object, [this_stance_com_from_stance_ankle_data_mean_free, this_stance_com_x_vel_midstance_data_mean_free], this_stance_fy_data_mean_free)
            xlabel('\Delta CoM from stance ankle'); ylabel('\Delta CoM vel'); zlabel('\Delta foot placement')
            title(['slopes = [' num2str(linear_model_slopes(1)) ', ' num2str(linear_model_slopes(2)) '], r^2 = ' num2str(fit_stats.rsquare)]);
        end    
        
        % fit fz regression model
        [fit_object, fit_stats] = fit([this_stance_com_from_stance_ankle_data_mean_free, this_stance_com_x_vel_midstance_data_mean_free], this_stance_fz_data_mean_free, 'poly11');
        fz_linear_models{i_stance} = [fit_object.p10, fit_object.p01];

        if visualize
            figure; 
            plot(fit_object, [this_stance_com_from_stance_ankle_data_mean_free, this_stance_com_x_vel_midstance_data_mean_free], this_stance_fz_data_mean_free)
            xlabel('\Delta CoM from stance ankle'); ylabel('\Delta CoM vel'); zlabel('\Delta foot placement')
            title(['slopes = [' num2str(linear_model_slopes(1)) ', ' num2str(linear_model_slopes(2)) '], r^2 = ' num2str(fit_stats.rsquare)]);
        end 
        
        % fit cop regression model
        [fit_object, fit_stats] = fit([this_stance_com_from_stance_ankle_data_mean_free, this_stance_com_x_vel_midstance_data_mean_free], this_stance_cop_data_mean_free, 'poly11');
        cop_linear_models{i_stance} = [fit_object.p10, fit_object.p01];

        if visualize
            figure; 
            plot(fit_object, [this_stance_com_from_stance_ankle_data_mean_free, this_stance_com_x_vel_midstance_data_mean_free], this_stance_cop_data_mean_free)
            xlabel('\Delta CoM from stance ankle'); ylabel('\Delta CoM vel'); zlabel('\Delta foot placement')
            title(['slopes = [' num2str(linear_model_slopes(1)) ', ' num2str(linear_model_slopes(2)) '], r^2 = ' num2str(fit_stats.rsquare)]);
        end 
    end
    ...
    fx_adjusted_data = zeros(size(fx_data)) * NaN;
    fx_adjusted_directions = fx_directions;
    fy_adjusted_data = zeros(size(fy_data)) * NaN;
    fy_adjusted_directions = fy_directions;
    fz_adjusted_data = zeros(size(fz_data)) * NaN;
    fz_adjusted_directions = fz_directions;
    cop_adjusted_data = zeros(size(cop_data)) * NaN;
    cop_adjusted_directions = cop_directions;
    ...
    for i_stretch = 1 : number_of_stretches
        ...
        this_stretch_fy_data = fy_data(:, i_stretch);
        this_stretch_fz_data = fz_data(:, i_stretch);
        this_stretch_cop_data = cop_data(:, i_stretch);
        ...
        for i_band = 1 : number_of_bands_per_stretch
            if ~isnan(this_stretch_step_placement_x_data(i_band))
                ...
                this_stretch_fx_data = this_stretch_fx_data(end);
                this_stretch_fy_data = this_stretch_fy_data(end);
                this_stretch_fz_data = this_stretch_fz_data(end);
                this_stretch_cop_data = this_stretch_cop_data(end);
                ...
                fx_delta = this_stretch_fx_data - fx_means(foot_index);
                fy_delta = this_stretch_fy_data - fy_means(foot_index);
                fz_delta = this_stretch_fz_data - fz_means(foot_index);
                cop_delta = this_stretch_cop_data - cop_means(foot_index);
                
                step_model_slopes = step_linear_models{foot_index};
                fx_model_slopes = fx_linear_models{foot_index};
                fy_model_slopes = fy_linear_models{foot_index};
                fz_model_slopes = fz_linear_models{foot_index};
                cop_model_slopes = cop_linear_models{foot_index};
                
                predicted_foot_placement_change = com_from_stance_ankle_delta * step_model_slopes(1) + com_vel_delta * step_model_slopes(2);
                ...
                predicted_fx_change = com_from_stance_ankle_delta * fx_model_slopes(1) + com_vel_delta * fx_model_slopes(2);
                actual_fx_change = fx_delta;
                fx_adjusted = actual_fx_change - predicted_fx_change;
                fx_adjusted_data(i_band, i_stretch) = fx_adjusted;
                
                predicted_fy_change = com_from_stance_ankle_delta * fy_model_slopes(1) + com_vel_delta * fy_model_slopes(2);
                actual_fy_change = fy_delta;
                fy_adjusted = actual_fy_change - predicted_fy_change;
                fy_adjusted_data(i_band, i_stretch) = fy_adjusted;
                
                predicted_fz_change = com_from_stance_ankle_delta * fz_model_slopes(1) + com_vel_delta * fz_model_slopes(2);
                actual_fz_change = fz_delta;
                fz_adjusted = actual_fz_change - predicted_fz_change;
                fz_adjusted_data(i_band, i_stretch) = fz_adjusted;
                
                predicted_cop_change = com_from_stance_ankle_delta * cop_model_slopes(1) + com_vel_delta * cop_model_slopes(2);
                actual_cop_change = cop_delta;
                cop_adjusted = actual_cop_change - predicted_cop_change;
                cop_adjusted_data(i_band, i_stretch) = cop_adjusted;
            end
        end
    end    
