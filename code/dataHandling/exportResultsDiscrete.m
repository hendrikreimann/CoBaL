...
function exportResultsDiscrete(varargin)
...
% source_label = 'Displacement';
    ...
    if isempty(parser.Results.source)
        source_file = 'results.mat';
    else
        source_file = parser.Results.source;
    end
    source_file_full = ['groupResults' filesep source_file];

    % determine output file
    if isempty(parser.Results.output)
        % no output specified, use source file name
        output_file = source_file;

        % remove .mat if present
        if length(output_file) > 4 && strcmp(output_file(end-3:end), '.mat')
            output_file = output_file(1:end-4);
        end
        
    else
        % use specified output file name
        output_file = parser.Results.output;
    end
    if length(output_file) < 4 || ~strcmp(output_file(end-3:end), '.csv')
        output_file = [output_file '.csv'];
    end
    output_file_full = ['groupResults' filesep output_file];
    
    % load data
    loaded_data = load(source_file_full);
    ...
    variables_to_export_discrete_table = study_settings.getTable('variables_to_export_discrete'); % TODO: finish switching to table here
    ...
    for i_variable = 1 : number_of_variables_to_export_discrete
        ...
        variable_index_in_data_cell = strcmp(this_variable_source, loaded_data.variable_names);
        ...
        data_cell = [data_cell this_variable_data_cell]; %#ok<AGROW> 
        data_header = [data_header this_variable_name]; %#ok<AGROW> 
    end
    ...
    cell2csv(output_file_full, export_cell);
