...
function joint_center_positions = ...
    ...
    transformations_reference = calculateMcsToWcsTransformations_new(marker_positions_reference, marker_labels_reference, segment_labels, subject_settings);
    transformations_current = calculateMcsToWcsTransformations_new(marker_positions_current, marker_labels_current, segment_labels, subject_settings);    
    
    
    % cervix
    T_reference_wcs_to_head_mcs = transformations_reference{strcmp(segment_labels, 'HEAD')}^(-1);
    cervix_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'CERVIXCOR')';
    ...
    % left shoulder
    T_reference_wcs_to_torso_mcs = transformations_reference{strcmp(segment_labels, 'TORSO')}^(-1);
    lshoulder_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'LSHOULDERCOR')';
    ...
    % right shoulder
    T_reference_wcs_to_torso_mcs = transformations_reference{strcmp(segment_labels, 'TORSO')}^(-1);
    rshoulder_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'RSHOULDERCOR')';
...
%     lwrist_cor_reference_wcs = extractMarkerData(joint_center_reference, joint_center_labels, 'LWRISTCOR')';
...
%     rwrist_cor_reference_wcs = extractMarkerData(joint_center_reference, joint_center_labels, 'RWRISTCOR')';
...
%     rwrist_cor_current_wcs = eye(3, 4) * T_current_hand_mcs_to_wcs * [rwrist_cor_hand_mcs; 1];
    ...
    lwrist_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'LWRISTCOR')'; % debugging: this does not depend on the wrist angle
    ...
    rwrist_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'RWRISTCOR')';
    ...
    lumbar_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'LUMBARCOR')';
...
%     T_reference_wcs_to_torso_mcs = transformations_reference{strcmp(segment_labels, 'TORSO')}^(-1);
%     lumbar_cor_reference_wcs = extractMarkerData(joint_center_reference, joint_center_labels, 'LUMBARCOR')';
...
%     lumbar_cor_current_wcs = mean([lumbar_cor_current_wcs_from_pelvis lumbar_cor_current_wcs_from_torso], 2);
    ...
    % left hip
    T_reference_wcs_to_pelvis_mcs = transformations_reference{strcmp(segment_labels, 'PELVIS')}^(-1);
    lhip_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'LHIPCOR')';
    ...
    % right hip
    T_reference_wcs_to_pelvis_mcs = transformations_reference{strcmp(segment_labels, 'PELVIS')}^(-1);
    rhip_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'RHIPCOR')';
    ...
    lankle_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'LANKLECOR')';
    ...
    rankle_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'RANKLECOR')';
    ...
    % left toes eef
    T_reference_wcs_to_foot_mcs = transformations_reference{strcmp(segment_labels, 'LFOOT')}^(-1);
    ltoes_eef_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'LTOESEEF')';
    ...
    % right toes eef
    T_reference_wcs_to_foot_mcs = transformations_reference{strcmp(segment_labels, 'RFOOT')}^(-1);
    rtoes_eef_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'RTOESEEF')';
    ...
    % left hand eef
    T_reference_wcs_to_hand_mcs = transformations_reference{strcmp(segment_labels, 'LHAND')}^(-1);
    lhand_eef_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'LHANDEEF')';
    ...
    % right hand eef
    T_reference_wcs_to_hand_mcs = transformations_reference{strcmp(segment_labels, 'RHAND')}^(-1);
    rhand_eef_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'RHANDEEF')';
    ...
    lelbow_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'LELBOWCOR')';
    ...
    % right elbow
    T_reference_wcs_to_forearm_mcs = transformations_reference{strcmp(segment_labels, 'RFOREARM')}^(-1);
    relbow_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'RELBOWCOR')';
    ...
    markers_and_cor_reference = [marker_positions_reference lhip_cor_reference_wcs' rhip_cor_reference_wcs'];
    markers_and_cor_positions = [marker_positions_current lhip_cor_current_wcs' rhip_cor_current_wcs'];
    markers_and_cor_labels = [marker_labels_reference {'LHIPCOR_x', 'LHIPCOR_y', 'LHIPCOR_z', 'RHIPCOR_x', 'RHIPCOR_y', 'RHIPCOR_z'}];
    ...
    lknee_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'LKNEECOR')'; % this seems wrong
    ...
    rknee_cor_reference_wcs = extractMarkerData(joint_center_positions_reference, joint_center_labels, 'RKNEECOR')';
...
if false
    % left knee
    LANK_current = extractMarkerData(marker_positions, marker_headers, 'LANK')'; % left ankle marker
    ...
    LKNE_current = extractMarkerData(marker_positions, marker_headers, 'LKNE')';
    LKNE_reference = extractMarkerData(marker_reference, marker_headers, 'LKNE')';
    LKNEECOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'LKNEECOR')';
    ...
    % right knee
    RANK_current = extractMarkerData(marker_positions, marker_headers, 'RANK')';
    rknee_axis_direction = normVector(rankle_cor_current_wcs - RANK_current);
    RKNE_current = extractMarkerData(marker_positions, marker_headers, 'RKNE')';
    RKNE_reference = extractMarkerData(marker_reference, marker_headers, 'RKNE')';
    RKNEECOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'RKNEECOR')';
    ...
    % left knee
    LKNE_reference = extractMarkerData(marker_reference, marker_headers, 'LKNE')';
    LHIPCOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'LHIPCOR')';
    LKNEECOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'LKNEECOR')';
    LANKLECOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'LANKLECOR')';
    LKNE_current = extractMarkerData(marker_positions, marker_headers, 'LKNE')';
    ...
    % right knee
    RKNE_reference = extractMarkerData(marker_reference, marker_headers, 'RKNE')';
    RHIPCOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'RHIPCOR')';
    RKNEECOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'RKNEECOR')';
    RANKLECOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'RANKLECOR')';
    RKNE_current = extractMarkerData(marker_positions, marker_headers, 'RKNE')';
    ...
    % left elbow
    LELB_reference = extractMarkerData(marker_reference, marker_headers, 'LELB')';
    LSHOULDERCOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'LSHOULDERCOR')';
    LELBOWCOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'LELBOWCOR')';
    LWRISTCOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'LWRISTCOR')';
    LELB_current = extractMarkerData(marker_positions, marker_headers, 'LELB')';
...
%     lelbow_cor_current_wcs = findHingeJointCenter(lshoulder_cor_current_wcs, lwrist_cor_current_wcs, LELB_current, r_marker, r_shoulder, r_wrist, 'negative');
    ...
    % right elbow
    RELB_reference = extractMarkerData(marker_reference, marker_headers, 'RELB')';
    RSHOULDERCOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'RSHOULDERCOR')';
    RELBOWCOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'RELBOWCOR')';
    RWRISTCOR_reference = extractMarkerData(joint_center_reference, joint_center_labels, 'RWRISTCOR')';
    RELB_current = extractMarkerData(marker_positions, marker_headers, 'RELB')';
