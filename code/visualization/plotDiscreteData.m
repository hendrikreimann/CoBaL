...
function plot_handles = plotDiscreteData(data, varargin)
    plot_handles = struct;
    ...
    if settings.spread.show
        if strcmp(settings.spread.style, 'box')
            plot_handles.spread = plotBox(settings, stats);
        elseif strcmp(settings.spread.style, 'violin')
            plot_handles.spread = plotViolin(settings, stats);
        else
    ...
    if settings.mean.show
        plot_handles.mean = plotMark(settings, stats, 'mean');
    end
    
    % plot median
    if settings.median.show
        plot_handles.median = plotMark(settings, stats, 'median');
    end
    
    % plot individual data
    if settings.individual_data.show
        plot_handles.data = plotIndividualData(settings, data, stats);
    end
...
function plot_handle = plotBox(settings, stats)
    plot_handle = struct;
    ...
    plot_handle.face = patch ...
      ( ...
        ...
        'FaceAlpha', settings.spread.face_alpha, ...
        'LineWidth', settings.spread.edge_linewidth, ...
        'HandleVisibility', 'off' ...
      );
  
    % wisks
    plot_handle.upper_range = plot ...
      ( ...
        settings.axes, ...
    ...
    plot_handle.lower_adjacent = plot ...
      ( ...
        settings.axes, ...
    ...
    plot_handle.upper_adjacent = plot ...
      ( ...
        settings.axes, ...
    	settings.abscissa+settings.width*[-0.25 0.25], ...
...
function plot_handle = plotViolin(settings, stats)
...
function plot_handle = plotMark(settings, stats, location)
